...
# Stel de tijdzone in op Eastern Time (ET)
et = pytz.timezone('America/Toronto')

def localize_and_convert(df, column, target_timezone):
    # Lokaliseer en converteer de tijdstempelkolom naar de doel-tijdzone
...
# Lokaliseer en converteer tijdstempels voor alle dataframes
...
# Bepaal de huidige datum en relevante datums
...
def filter_and_calculate(df_filling_infeed, df_transplanting_infeed, df_bench_weight, date=None, start_date=None, end_date=None):
    # Filter data op basis van specifieke datum of datumbereik
    if date:
        ...
        data_filling_infeed = df_filling_infeed[(df_filling_infeed['timestamp'].dt.date >= start_date) & (df_filling_infeed['timestamp'].dt.date < end_date)]
        data_transplanting_infeed = df_transplanting_infeed[(df_transplanting_infeed['timestamp'].dt.date >= start_date) & (df_transplanting_infeed['timestamp'].dt.date < end_date)]
        data_bench_weight = df_bench_weight[(df_bench_weight['crate weight timestamp'].dt.date >= start_date) & (df_bench_weight['crate weight timestamp'].dt.date < end_date)]

    # Verwijder rijen zonder gewaskoppeling
    data_bench_weight = data_bench_weight[data_bench_weight['crop name'] != 'Crop not found']

    # Bereken unieke transplant jobs
    unique_transplant_jobs = data_transplanting_infeed.drop_duplicates(subset=['transplant job bench name'])

    # Bereken totaal geoogst gewicht en aantal gebeurtenissen
    ...
    # Combineer uitgifte-evenementen
    ...
    return {
        ...
        'transplanting_infeed_15MIN': calculate_15MIN_frequency(data_transplanting_infeed, date, start_date, 'timestamp'),
        ...
        'data_bench_weight_15MIN': calculate_15MIN_frequency(data_bench_weight, date, start_date, 'crate weight timestamp'),
...
def combine_outfeed_events(df_bench_weight, df_unique_transplant_jobs):
    # Combineer uitgiftegegevens uit twee bronnen
    df_bench_weight = df_bench_weight[['crate weight timestamp']].rename(columns={'crate weight timestamp': 'timestamp'})
...
def calculate_15MIN_frequency(df, date=None, start_date=None, timestamp_column='timestamp'):
    # Groepeer gegevens in tijdsintervallen van 15 minuten
    df = df.copy()
    if date:
        df['time_group'] = df[timestamp_column].dt.floor('15min')
    elif start_date:
...
def calculate_change(new_value, old_value):
    # Bereken verandering tussen nieuwe en oude waarde
...
def get_changes(new_metrics, old_metrics):
    # Vergelijk nieuwe en oude metrics om veranderingen te berekenen
    changes = {}
...
# Bereken statistieken voor specifieke datums en perioden
metrics = {
    ...
    'week': filter_and_calculate(df_filling_infeed, df_transplanting_infeed, df_bench_weight, start_date=last_week, end_date=today),
    'day_before_yesterday': filter_and_calculate(df_filling_infeed, df_transplanting_infeed, df_bench_weight, date=day_before_yesterday),
    'two_weeks_ago': filter_and_calculate(df_filling_infeed, df_transplanting_infeed, df_bench_weight, start_date=two_weeks_ago, end_date=last_week),
}

# Bereken veranderingen ten opzichte van eerdere perioden
